...
class Block:
    ...
    def check_wall_collisions(self, entity):
        ...
        for block in self.__rect:
            ...
            # left collision
            elif entity.getDirection() == "left" and block[0] + entity.getRadius() <= entity.getX() <= block[0] + block[
                2] + entity.getRadius() and block[1] <= entity.getY() <= block[1] + block[3]:
                entity.setX(block[0] + block[2] + entity.getRadius())
            # top collision
            elif entity.getDirection() == "up" and block[0] <= entity.getX() <= block[0] + block[2] and block[
                1] + entity.getRadius() <= entity.getY() <= block[1] + block[3] + entity.getRadius():
                entity.setY(block[1] + block[2] + entity.getRadius())
            # bottom collision
            elif entity.getDirection() == "down" and block[0] <= entity.getX() <= block[0] + block[2] and block[
                1] - entity.getRadius() <= entity.getY() <= block[1] + block[3] - entity.getRadius():
